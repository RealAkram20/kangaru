...
| `POST api/v1/driver-applications` (public.driver-applications.store) | D | throttle 5/min; **202 with `data: null`** — no id, no echo; duplicates accepted silently (no oracle). | public | no |
...
| `POST api/v1/customer/trips/{trip}/rating` (customer.trips.rating.store) | C | controller compares `trip.customer_id`, but **the `{trip}` binding fails closed under TenantScope for a Customer, so it is 404 for everyone — endpoint dead** (F5, proved). | 404 cross-tenant (tenant-bound binding, proved) | no |
...
| `POST api/v1/trips/{trip}/transitions` (trips.transitions.store) | A | `TripPolicy::transition` → `any` / `finance` (disputed, closed) / `own` (journey states, assigned driver only). | 404 cross-tenant (tenant-bound binding, proved) | yes |
...
|---|---|---|---|
| **3.1** `Ci/RoutePolicyCensusTest` | every route has a census row and idiom; every non-public route carries `auth:sanctum` or `auth:customer`; every public route carries a throttle; every staff route but the seven `auth/*` self-service ones runs `IdentifyTenant` | 172 routes · 11 public · 3 idiom-B · 154 sanctum (147 + 7) · 7 customer-guard | (structural — a route added without a row fails it; asserted by construction rather than mutated) |
...
| F5 | Medium (completeness) | **`POST /customer/trips/{trip}/rating` is dead — 404 for every customer, including on their own completed trip.** Proved. `{trip}` resolves through `BelongsToTenant::resolveRouteBinding`, which drops `TenantScope` only for a platform *User*; the actor is a Customer, customer routes run no `IdentifyTenant`, so the scope fails closed and the binding 404s before `TripRatingController` runs (the message is the framework's, not the controller's "No such trip."). No test in the suite hits this route. ADR-0030's loop is open at the backend. Pinned in 3.4 so the day it is fixed the test flips to 201 on purpose. **Fix:** resolve `{trip}` with `Trip::forCustomer($customer)` in the controller (as `CustomerRideController` does), or teach the resolver about the customer guard. Also a W1-f row: seeded "Trip ratings — yes" is wrong. | Trips, Customers |
