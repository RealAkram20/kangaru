...
## Amended 2026-08-18: a development switch, inert in production

The seeded Finance and Super Admin accounts carry a known TOTP secret, so
every sign-in as either of them during development costs a code generated by
hand. The owner asked for the factor to be off while the platform is still
being built.

`config/mfa.php` (`MFA_ENABLED`) turns off **two things and no others**: the
demand that an MFA-required role enrol, and the challenge at sign-in. It does
not delete a secret, does not clear a recovery code, and does not change what
`roles.requires_mfa` says — so setting it back to true restores exactly the
accounts that were protected, with the same secrets.

**It cannot be switched off in production.**
`App\Support\Auth\MfaRequirement::inForce()` checks the environment before it
reads the config value, so `MFA_ENABLED=false` copied into a live `.env` is inert. A switch
that could disable the second factor on a running fleet by way of a copied
file would be worse than no switch, and AGENTS.md's requirement for the roles
that move money stands unchanged.

Everything below is unamended: it is what is in force whenever the factor is
in force, which is always, in production.
