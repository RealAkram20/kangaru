...
{
    public function __construct(private readonly AllocationLookup $allocations) {}
    ...
    public function forBooking(Booking $booking): Collection
    {
        ...
        return $vehicles
            ->map(function (Vehicle $vehicle) use ($booking, $contracted, $on) {
                ...
                return [
                    ...
                    // overridden.
                    'requires_override_reason' => ! $blocked && ! $allocated && $contracted->isNotEmpty(),
                    'note' => $this->noteFor($blocked, $allocated),
                ];
            })
     ...
     */
    private function noteFor(bool $blocked, bool $allocated): ?string
    {
        return match (true) {
